...
class FilaMariscalesApp {
    ...
    async loadCarousel() {
        ...
        try {
            // Limpiar carousel existente antes de recargar
            const existingCarousel = container.querySelector('.carousel');
            if (existingCarousel) {
                // Destruir instancia de Bootstrap si existe
                const bsCarousel = bootstrap.Carousel.getInstance(existingCarousel);
                if (bsCarousel) {
                    bsCarousel.dispose();
                }
            ...
            if (data && data.slides && Array.isArray(data.slides) && data.slides.length > 0) {
                // Filtrar slides válidos (con imagen_url)
                const validSlides = data.slides.filter(slide => slide.imagen_url && slide.imagen_url.trim() !== '');
                
                if (validSlides.length > 0) {
                    const validData = { ...data, slides: validSlides };
                    this.renderCarousel(validData, container);
                } else {
    ...
    renderCarousel(data, container) {
        ...
        // Verificar que el carrusel se creó correctamente antes de configurar eventos
        const createdCarousel = document.getElementById(carouselId);
        if (createdCarousel) {
            // Configurar eventos personalizados
            this.setupCarouselEvents(carouselId, config);
        } else {
            console.error('Error: No se pudo crear el carrusel con ID:', carouselId);
        }
    }
    ...
    setupCarouselEvents(carouselId, config) {
        ...
        if (!carousel) {
            console.warn('Carrusel no encontrado:', carouselId);
            return;
        }

        // Esperar a que el carrusel esté completamente renderizado
        setTimeout(() => {
            try {
                ...
                if (config.pause_on_hover) {
                    carousel.addEventListener('mouseenter', () => {
                        try {
                            ...
                            console.warn('Error pausando carrusel:', error);
                        }
                    });
                    ...
                    carousel.addEventListener('mouseleave', () => {
                        try {
                            ...
                            console.warn('Error reanudando carrusel:', error);
                        }
                    });
                }

                // Efectos de transición personalizados con verificación robusta de elementos
                ...
                carousel.addEventListener('slid.bs.carousel', (e) => {
                    try {
                        ...
                        // Limpiar clases de animación con verificación robusta
                ...
                console.warn('Error configurando eventos del carrusel:', error);
            }
        }, 100);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Función global para recargar carousel desde el admin
    window.reloadCarousel = function() {
        if (app && app.loadCarousel) {
            app.loadCarousel();
            console.log('Carrusel recargado desde el admin');
        }
    };
    ...
    if (typeof BroadcastChannel !== 'undefined') {
        ...
        // Canal para actualizaciones del carousel
        const carouselChannel = new BroadcastChannel('carousel-update');
        carouselChannel.addEventListener('message', (event) => {
            if (event.data.type === 'carousel-updated') {
                console.log('Recibida actualización del carrusel desde otra pestaña');
    ...
    window.addEventListener('storage', (event) => {
        ...
        if (event.key === 'carousel-updated') {
            console.log('Recibida actualización del carrusel desde localStorage');
            window.reloadCarousel();
        }
    });
